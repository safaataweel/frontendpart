      ...
      try {
        ...
        if (axios.isAxiosError(error)) {
          ...
          if (axiosError.response?.status === 401) {
            ...
            console.error('Error fetching document:', axiosError.message);
          }
        } else {
    ...
    <div className="min-h-screen bg-gradient-to-r from-pink-100 to-yellow-100 p-6 flex flex-col">
      <header className="flex items-center justify-between mb-6 bg-white shadow-md p-4 rounded-lg">
        <div className="flex items-center">
          <FaPen className="text-pink-600 text-3xl mr-3" title="Edit Document" aria-label="Edit Document" />
          <h1 className="text-4xl font-extrabold font-serif text-pink-700">Document Editor</h1>
        </div>
        <button
